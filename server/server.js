...
app.get('/todos', authenticate, (req, res) => {
  //add the _creator property to find to return todos that were actually created
  Todo.find({
    _creator: req.user._id
  }).then((todos) => {
    res.send({todos});
...
app.get('/todos/:id', authenticate, (req, res) => {
  ...
  //make sure to query by ID and property, allowing others to not access someone else's todo
  Todo.findOne({
    _id: id,
    _creator:req.user._id
  }).then((todo) => {
